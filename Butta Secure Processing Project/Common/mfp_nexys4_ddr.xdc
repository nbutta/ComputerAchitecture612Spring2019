...
#Our JTAG TCK is forced to live on a port J*[4] because of our physical adapter.
#We don't have a clock-pin available so we force the acceptance of sub-optimal routing
set_property -dict {PACKAGE_PIN H14 IOSTANDARD LVCMOS33} [get_ports {JB[4]}]
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets {JB[4]}]

#JTAG trst
set_property -dict {PACKAGE_PIN E16 IOSTANDARD LVCMOS33} [get_ports {JB[7]}]
set_property PULLUP true [get_ports {JB[7]}]

set_false_path -from [get_clocks jtag_tck] -to [get_clocks -of_objects [get_pins clk_wiz_0/inst/plle2_adv_inst/CLKOUT0] -filter {IS_GENERATED && MASTER_CLOCK == CLK100MHZ}]
set_false_path -from [get_clocks jtag_tck] -to [get_clocks -of_objects [get_pins clk_wiz_0/inst/plle2_adv_inst/CLKOUT0] -filter {IS_GENERATED && MASTER_CLOCK == sys_clk_pin}]
